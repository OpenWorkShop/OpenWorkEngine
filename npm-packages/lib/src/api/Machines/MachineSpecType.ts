...
export function getMachineSpecTypeUnits(mst: MachineSpecType | string): string | undefined {
  ...
  if (speed.includes(str)) return 'mm/min';
  if (temp.includes(str)) return '°C';
  if (rpm.includes(str)) return 'rpm';
  if (nm.includes(str)) return 'nm';
  if (mm.includes(str)) return 'mm';
  if (watts.includes(str)) return 'W';
  if (volts.includes(str)) return 'V';
  if (amps.includes(str)) return 'A';
  return undefined;
}
