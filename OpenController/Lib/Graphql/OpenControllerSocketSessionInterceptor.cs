...
namespace OpenWorkEngine.OpenController.Lib.Graphql {
  public class OpenControllerSocketSessionInterceptor : DefaultSocketSessionInterceptor {
    public override async ValueTask<ConnectionStatus> OnConnectAsync(
      ...
      if (message.Payload?["token"] is string token) {
        ClaimsPrincipal? principal = await connection.HttpContext.ClaimOpenControllerIdentity(token);
