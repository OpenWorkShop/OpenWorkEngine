...
import {IHaveVisualizerPreferences, IVisualizerPreferences, ViewPlane} from '../GWiz';
import {OpenWorkShopIcon} from '../../components/';
import {useTrans} from '../Context';
import useStyles, {tooltipDelay} from './Styles';

type Props = IHaveVisualizerPreferences;

const ViewModeSelect: React.FunctionComponent<Props> = (props) => {
  const t = useTrans();
  const classes = useStyles();
  const visualizerPreferences: IVisualizerPreferences = props.visualizerPreferences;
  const { viewPlane, setViewPlane } = visualizerPreferences;
